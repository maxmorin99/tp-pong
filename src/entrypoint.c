...
action_t qlearn_get_action(qlearn_t *qlearn, state_t S){
    ...
    if (rand() <= RAND_MAX * qlearn->epsilon)
    {
        a = rand() % qlearn->n_actions;
    }
    else
    {
        a = 0;
...
void qlearn_update_qvalue(qlearn_t *qlearn, state_t S, action_t a, reward_t R, qvalue_t Q_max){
    ...
    Q += (qlearn->learning_rate * (R + qlearn->discount_factor * Q_max - Q));
...
void raylib_start(void){
    ...
    //qlearn_load_table(qlearn, "qlearn_data.txt");
    ...
    while (!WindowShouldClose()) 
    {
        ...
        {
            // Load tarined model
            if(IsKeyDown(KEY_SPACE) && !bModelLoaded)
            {
                qlearn_load_table(qlearn, "qlearn_data.txt");
                bModelLoaded = true;
            }

            // get random or best action depending on epsilon
            action_t a = qlearn_get_action(qlearn, S);

            // get new game state after performing action
            qvalue_t S_new = qlearn_apply_action(qlearn, a);

            // learn only if e is not 0
            if(qlearn->epsilon > 0.00000001f)
            {
                // Get reward from last performed action
                reward_t reward = qlearn->get_reward();

                // Get Q_Max from the new game state
                qvalue_t Q_max = qlearn_get_max_qvalue(qlearn, S_new);

                // Update QValue of the last game state
        ...
        // save here since the loop never exits
        qlearn_save_table(qlearn, "qlearn_data.txt");
